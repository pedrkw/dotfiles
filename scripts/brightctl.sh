...
if [[ "$1" == "-h" ]]; then
    echo "Usage:"
    echo "  ./brightness.sh increase       -> Increases brightness by 10%"
    echo "  ./brightness.sh decrease       -> Decreases brightness by 10%"
    echo "  ./brightness.sh <value>        -> Sets brightness to a specific value (0-100)"
    echo "  ./brightness.sh                -> Displays the current brightness"
    echo "  ./brightness.sh -h             -> Shows this help message"
    exit 0
fi
